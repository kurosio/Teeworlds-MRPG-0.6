...
void CGuildManager::ShowMembershipList(int ClientID) const
{
	...
	// Membership list
	CVoteWrapper(ClientID).Add("\u2635 Membership list of {STR}", pGuild->GetName());
	...
	for(auto& pIterMember : pGuild->GetMembers()->GetContainer())
	{
		...
		CVoteWrapper VMember(ClientID, VWFLAG_UNIQUE|VWFLAG_BSTYLE_SIMPLE, "{STR} {STR} Deposit: {VAL}", pMember->GetRank()->GetName(), pNickname, pMember->GetDeposit());
...
void CGuildManager::ShowMenu(int ClientID) const
{
	...
	// Guild information
	CVoteWrapper VInfo(ClientID, VWFLAG_DEFAULT_OPEN|VWFLAG_BSTYLE_SIMPLE, "\u2747 Information about {STR}", pGuild->GetName());
	...
	// Guild deposit
	CVoteWrapper VDeposit(ClientID, VWFLAG_DEFAULT_OPEN, "\u2727 Your: {VAL} | Bank: {VAL} golds", pPlayer->GetItem(itGold)->GetValue(), pGuild->GetBank()->Get());
	VDeposit.AddOption("GUILD_DEPOSIT_GOLD", "Deposit. (Amount in a reason)");
	VDeposit.AddLine();

	// Guild management
	CVoteWrapper VManagement(ClientID, VWFLAG_DEFAULT_OPEN, "\u262B Guild Management");
	...
	if(HasHouse)
	{
		...
		// House management
		CVoteWrapper VHouse(ClientID, VWFLAG_DEFAULT_OPEN, "\u2302 House Management");
		...
		{
			CVoteWrapper VDoors(ClientID, VWFLAG_DEFAULT_OPEN, "\u2743 House has {VAL} controlled door's", (int)pHouse->GetDoorManager()->GetContainer().size());
		...
		{
			CVoteWrapper VPlantZones(ClientID, VWFLAG_DEFAULT_OPEN, "\u2741 House has {VAL} plant zone's", (int)pHouse->GetPlantzonesManager()->GetContainer().size());
	...
	// Guild disband
	CVoteWrapper VDisband(ClientID, VWFLAG_DEFAULT_OPEN, "\u2716 Disband guild");
	...
	// Guild upgrades
	CVoteWrapper VUpgrades(ClientID, VWFLAG_DEFAULT_OPEN, "\u2730 Guild upgrades");
...
void CGuildManager::ShowRanksSettings(int ClientID) const
{
	...
	CVoteWrapper VRanks(ClientID, VWFLAG_DEFAULT_CLOSE|VWFLAG_BSTYLE_SIMPLE, "\u2324 Rank management (Information)");
	...
	CVoteWrapper VRankSettings(ClientID, VWFLAG_DEFAULT_OPEN, "\u2743 Rank management (Settings)");
	...
	for(auto pRank : pPlayer->Account()->GetGuild()->GetRanks()->GetContainer())
	{
		...
		CVoteWrapper VRank(ClientID, VWFLAG_UNIQUE|VWFLAG_BSTYLE_SIMPLE, "\u2742 Rank [{STR}] {STR}", pRank->GetName(), StrAppendRankInfo.c_str());
...
void CGuildManager::ShowPlantZone(int ClientID, int PlantzoneID) const
{
	...
	// information
	CVoteWrapper VPlantInfo(ClientID, VWFLAG_DEFAULT_CLOSE, "\u2741 Plant zones information");
	...
	CVoteWrapper VPlantSettings(ClientID, VWFLAG_BSTYLE_STRICT_BOLD);
	VPlantSettings.Add("\u2741 Plant zone: {STR}", pPlantzone->GetName());
	VPlantSettings.Add("Planted: {STR}", pItem->GetName());
	VPlantSettings.AddLine();

	// items
	CVoteWrapper VPlantItems(ClientID, VWFLAG_DEFAULT_OPEN, "\u2741 Possible items for planting");
...
void CGuildManager::ShowBuyHouse(int ClientID, CGuildHouseData* pHouse) const
{
	...
	CVoteWrapper VHouseInfo(ClientID, VWFLAG_DEFAULT_CLOSE, "Information guild house");
	...
	CVoteWrapper VHouse(ClientID, VWFLAG_DEFAULT_OPEN, "House detail information");
	if(!pHouse)
	{
	...
	if(!pHouse->IsPurchased())
	{
		...
		CGuildData* pGuild = pPlayer->Account()->GetGuild();
		CVoteWrapper VBuyHouse(ClientID, VWFLAG_DEFAULT_OPEN, "Guild bank: {VAL} gold", pGuild->GetBank()->Get());
...
void CGuildManager::ShowLogs(int ClientID) const
{
	...
	CVoteWrapper VLogger(ClientID, VWFLAG_DEFAULT_OPEN|VWFLAG_BSTYLE_SIMPLE, "Activity log settings");
	...
	// Logs
	char aBuf[128];
	CVoteWrapper VLogs(ClientID, VWFLAG_DEFAULT_CLOSE, "Logs");
