...
void CAethernetManager::ShowMenu(CCharacter* pChar) const
{
	...
	// Default aether menu
	CVoteWrapper VAether(ClientID, VWFLAG_BSTYLE_STRICT_BOLD, "Aethernet information");
	...
	for(auto& [WorldID, vAethers] : s_vpAetherSortedList)
	{
		...
		CVoteWrapper VAetherElem(ClientID, VWFLAG_DEFAULT_OPEN | VWFLAG_BSTYLE_SIMPLE, "{STR} : Aethernet", Server()->GetWorldName(WorldID));
