...
bool CAccountManager::OnHandleMenulist(CPlayer* pPlayer, int Menulist)
{
	...
	if(Menulist == MENU_SETTINGS)
	{
		...
		// information
		CVoteWrapper VSettingsInfo(ClientID, VWFLAG_SEPARATE_CLOSED, "Settings Information");
		...
		// game settings
		CVoteWrapper VMainSettings(ClientID, VWFLAG_SEPARATE_OPEN, "\u2699 Main settings");
		...
		// equipment modules
		CVoteWrapper VModulesSettings(ClientID, VWFLAG_SEPARATE_OPEN, "\u2694 Modules settings");
	...
	if(Menulist == MENU_SETTINGS_LANGUAGE_SELECT)
	{
		...
		CVoteWrapper VLanguageInfo(ClientID, VWFLAG_SEPARATE_CLOSED, "Languages Information");
		...
		CVoteWrapper VLanguage(ClientID, VWFLAG_STYLE_STRICT_BOLD);
		VLanguage.Add("Active language: [{STR}]", pPlayerLanguage);
		VLanguage.AddLine();

		// languages
		CVoteWrapper VLanguages(ClientID, VWFLAG_SEPARATE_OPEN, "Available languages");
