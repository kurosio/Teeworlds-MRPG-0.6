...
void CWarehouse::Init(const std::string& Name, const DBSet& Type, const nlohmann::json& Data, vec2 Pos, int Currency, int WorldID)
{
	m_Flags = WF_NONE;
	m_Name = Name;
	m_Pos = Pos;
...
void CWarehouse::InitData(const DBSet& Type, const nlohmann::json& JsonData)
{
	// check properties exist
	dbg_assert(!JsonData.empty(), "The data json is empty");
	...
	{
		// by collections
		if(JsonData.contains("items_collections"))
		{
			for(const auto& pItem : JsonData["items_collections"])
			{
		...
		{
			for(const auto& pItem : JsonData["items"])
			{
	...
	if(m_Flags & WF_STORAGE)
	{
		...
		dbg_assert(JsonData.contains("storage"), "The warehouse is flagged as storage, but no storage data provided");

		// initialize storage
		const auto& JsonStorage = JsonData["storage"];
