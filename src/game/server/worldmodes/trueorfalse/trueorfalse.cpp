...
// tile types for zones
constexpr int TILE_TOF_LOBBY   = 146; // TILE_QUEST_BOARD
constexpr int TILE_TOF_TRUE    = 24; // TILE_SW_HOUSE_ZONE
constexpr int TILE_TOF_FALSE   = 25; // TILE_SW_ACTION_ZONE
...
void CGameControllerTrueOrFalse::Tick()
{
	...
	// phase 1: show question for 8 seconds, walls closed
	...
	// phase 2: countdown after walls open
	if(m_QuestionActive && m_WallsOpen)
	{
	...
	// phase 3: show results, then teleport everyone back to neutral
	if(m_ResultsActive)
	{
		if(m_ResultsTick > 0)
		{
...
void CGameControllerTrueOrFalse::KillWrongZonePlayers()
{
	for(int i = 0; i < MAX_PLAYERS; i++)
	{
		...
		// lobby (21) = safe, correct answer zone = safe, wrong zone or neutral = die
...
void CGameControllerTrueOrFalse::ConTrueOrFalseStart(IConsole::IResult* pResult, void* pUser)
{
	...
	if(Active == 1)
	{
		...
		// spawn walls and teleport players once
		pSelf->SpawnWalls();
...
// find seconds (second to last token)
...
const char* pAnswer = aAnswer;
	...
	pSelf->GS()->Console()->PrintFormat(IConsole::OUTPUT_LEVEL_STANDARD, "trueorfalse",
		"DEBUG question='%s' seconds=%d answer='%s' CountdownTick=%d", pQuestion, Seconds, pAnswer, pSelf->m_CountdownTick);

	// spawn fresh walls for this question
	pSelf->DestroyWalls();
	pSelf->SpawnWalls();
}
