...
GUILD_MEMBER_RESULT CGuildRequestsController::Accept(int UserID, const CGuildMemberData* pFromMember)
{
	...
	if(Result == GUILD_MEMBER_RESULT::SUCCESSFUL && pFromMember)
	{
		...
		m_pGuild->GetHistory()->Add("'%s' accepted invitation from '%s'.", pByNickname, pFromNickname);
...
void CGuildRequestsController::Deny(int UserID, const CGuildMemberData* pFromMember)
{
	...
	if(Iter != m_aRequestsJoin.end())
	{
		...
		if(pFromMember)
		{
			...
			// Add a history entry and send a guild chat message
			m_pGuild->GetHistory()->Add("'%s' denied invitation from '%s'.", pByNickname, pFromNickname);
