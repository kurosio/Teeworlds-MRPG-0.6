...
GUILD_MEMBER_RESULT CGuildRequestsManager::Request(int FromUID)
{
	...
	m_pGuild->GetLogger()->Add("invitation to join from '%s'.", pFromNickname);
...
GUILD_MEMBER_RESULT CGuildRequestsManager::Accept(int UserID, const CGuildMemberData* pFromMember)
{
	...
	if(Result == GUILD_MEMBER_RESULT::SUCCESSFUL && pFromMember)
	{
		...
		const char* pFromNickname = Instance::GetServer()->GetAccountNickname(UserID);
		const char* pByNickname = Instance::GetServer()->GetAccountNickname(pFromMember->GetAccountID());

		// Add a history entry and send a guild chat message
		m_pGuild->GetLogger()->Add("'%s' accepted invitation from '%s'.", pByNickname, pFromNickname);
...
void CGuildRequestsManager::Deny(int UserID, const CGuildMemberData* pFromMember)
{
	...
	if(Iter != m_aRequestsJoin.end())
	{
		...
		if(pFromMember)
		{
			...
			// Add a history entry and send a guild chat message
			m_pGuild->GetLogger()->Add("'%s' denied invitation from '%s'.", pByNickname, pFromNickname);
