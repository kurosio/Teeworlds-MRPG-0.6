...
void CQuestPathFinder::Tick()
{
	...
	if (m_pPlayer->GetQuest(QuestID).m_Step != Step || m_pPlayer->GetQuest(QuestID).GetState() != QuestState::ACCEPT || m_pPlayer->GetQuest(QuestID).m_aPlayerSteps[m_SubBotID].m_StepComplete)
	{
