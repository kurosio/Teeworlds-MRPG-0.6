...
public:
	...
	vector2_base() : x(), y() { }
	vector2_base(T nx, T ny) : x(nx), y(ny) { }
	...
	vector2_base& operator -=(const vector2_base& v) { x -= v.x; y -= v.y; return *this; }
	vector2_base& operator *=(const T v) { x *= v; y *= v; return *this; }
	vector2_base& operator *=(const vector2_base& v) { x *= v.x; y *= v.y; return *this; }
	vector2_base& operator /=(const T v) { x /= v; y /= v; return *this; }
	vector2_base& operator /=(const vector2_base& v) { x /= v.x; y /= v.y; return *this; }
...
template<typename T>
T squared_dist(const vector2_base<T>& a, const vector2_base<T>& b)
{
	T dx = a.x - b.x;
	T dy = a.y - b.y;
	return dx * dx + dy * dy;
...
template<typename T>
vector2_base<T> closest_point_on_line(const vector2_base<T>& line_point0, const vector2_base<T>& line_point1, const vector2_base<T>& target_point)
{
...
template<typename T>
bool closest_point_on_line(vector2_base<T> line_pointA, vector2_base<T> line_pointB, vector2_base<T> target_point, vector2_base<T>& out_pos)
{
