...
void str_translation_cyrlic_to_latin(char* str)
{
	...
	for(int i = 0; i < std::size(translate_from); i++)
	{
...
bool shell_register_protocol(const char *protocol_name, const char *executable, bool *updated)
{
	...
	const LRESULT result_value_empty = RegSetValueEx(handle_subkey_protocol, LPCSTR("URL Protocol"), 0, REG_SZ, (BYTE *)L"", sizeof(wchar_t));
