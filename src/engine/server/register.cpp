...
const char* CRegister::ProtocolToScheme(int Protocol)
{
	switch(Protocol)
	{
		case PROTOCOL_IPV6: return "tw-0.6+udp://";
		case PROTOCOL_IPV4: return "tw-0.6+udp://";
	}
...
const char* CRegister::ProtocolToString(int Protocol)
{
	switch(Protocol)
	{
		case PROTOCOL_IPV6: return "tw0.6/ipv6";
		case PROTOCOL_IPV4: return "tw0.6/ipv4";
	}
	dbg_assert(false, "invalid protocol");
	dbg_break();
}
...
const char* CRegister::ProtocolToSystem(int Protocol)
{
	switch(Protocol)
	{
		case PROTOCOL_IPV6: return "register/6/ipv6";
		case PROTOCOL_IPV4: return "register/6/ipv4";
	}
	dbg_assert(false, "invalid protocol");
	dbg_break();
}
...
CRegister::CRegister(CConfig* pConfig, IConsole* pConsole, IEngine* pEngine, IHttp* pHttp, int ServerPort, unsigned SixupSecurityToken) :
	...
	m_ServerPort(ServerPort),
	m_aProtocols {
		CProtocol(this, PROTOCOL_IPV6),
		CProtocol(this, PROTOCOL_IPV4),
	}
{
...
void CRegister::Update()
{
	...
	{
		bool Ipv6 = m_aProtocolEnabled[PROTOCOL_IPV6];
		bool Ipv4 = m_aProtocolEnabled[PROTOCOL_IPV4];
		if(Ipv6 && Ipv4)
		{
...
void CRegister::OnConfigChange()
{
	...
	else
	{
		...
		while((pProtocols = str_next_token(pProtocols, ",", aBuf, sizeof(aBuf))))
		{
			...
			{
				m_aProtocolEnabled[PROTOCOL_IPV6] = true;
			}
			else if(str_comp(aBuf, "ipv4") == 0)
			{
				m_aProtocolEnabled[PROTOCOL_IPV4] = true;
			}
	...
	{
		m_aProtocolEnabled[PROTOCOL_IPV6] = false;
		m_NumExtraHeaders = 0;
