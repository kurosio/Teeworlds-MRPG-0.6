...
void CNetBan::Init(IConsole *pConsole, IStorageEngine *pStorage)
{
	m_pConsole = pConsole;
	m_pStorage = pStorage;
	m_BanAddrPool.Reset();
...
void CNetBan::ConBansSave(IConsole::IResult *pResult, void *pUser)
{
	...
	char aBuf[256];
	IOHANDLE File = pThis->Storage()->OpenFile(pResult->GetString(0), IOFLAG_WRITE, IStorageEngine::TYPE_SAVE);
	if(!File)
	{
