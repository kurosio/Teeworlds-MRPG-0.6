...
// Record
int CDemoRecorder::Start(class IStorageEngine *pStorage, class IConsole *pConsole, const char *pFilename, const char *pNetVersion, const char *pMap, const SHA256_DIGEST &Sha256, unsigned Crc, const char *pType, unsigned MapSize, unsigned char *pMapData, IOHANDLE MapFile, DEMOFUNC_FILTER pfnFilter, void *pUser)
{
	...
	IOHANDLE DemoFile = pStorage->OpenFile(pFilename, IOFLAG_WRITE, IStorageEngine::TYPE_SAVE);
	if(!DemoFile)
	{
		if(m_pConsole)
		{
	...
	if(!pMapData && !MapFile)
	{
		...
		MapFile = pStorage->OpenFile(aMapFilename, IOFLAG_READ, IStorageEngine::TYPE_ALL);
		if(!MapFile)
		{
		...
		if(!MapFile)
		{
			...
			if(pStorage->FindFile(aMapFilename, "maps", IStorageEngine::TYPE_ALL, aBuf, sizeof(aBuf)))
				MapFile = pStorage->OpenFile(aBuf, IOFLAG_READ, IStorageEngine::TYPE_ALL);
		}
		if(!MapFile)
		{
			if(m_pConsole)
...
int CDemoPlayer::Load(class IStorageEngine *pStorage, class IConsole *pConsole, const char *pFilename, int StorageType)
{
...
unsigned char *CDemoPlayer::GetMapData(class IStorageEngine *pStorage)
{
	if(!m_MapInfo.m_Size)
		return nullptr;
...
bool CDemoPlayer::ExtractMap(class IStorageEngine *pStorage)
{
	...
	// save map
	IOHANDLE MapFile = pStorage->OpenFile(aMapFilename, IOFLAG_WRITE, IStorageEngine::TYPE_SAVE);
	if(!MapFile)
	{
		free(pMapData);
		return false;
...
{
	IStorageEngine::StripPathAndExtension(m_aFilename, pBuffer, BufferSize);
}

bool CDemoPlayer::GetDemoInfo(IStorageEngine *pStorage, IConsole *pConsole, const char *pFilename, int StorageType, CDemoHeader *pDemoHeader, CTimelineMarkers *pTimelineMarkers, CMapInfo *pMapInfo, IOHANDLE *pFile, char *pErrorMessage, size_t ErrorMessageSize) const
{
...
void CDemoEditor::Init(const char *pNetVersion, class CSnapshotDelta *pSnapshotDelta, class IConsole *pConsole, class IStorageEngine *pStorage)
{
...
void CDemoEditor::Slice(const char *pDemo, const char *pDst, int StartTick, int EndTick, DEMOFUNC_FILTER pfnFilter, void *pUser)
{
	...
	if(DemoPlayer.Load(m_pStorage, m_pConsole, pDemo, IStorageEngine::TYPE_ALL_OR_ABSOLUTE) == -1)
		return;
