...
bool CConfigManager::Save()
{
	...
	m_ConfigFile = m_pStorage->OpenFile(IStorageEngine::FormatTmpPath(aConfigFileTmp, sizeof(aConfigFileTmp), CONFIG_FILE), IOFLAG_WRITE, IStorageEngine::TYPE_SAVE);
...
#undef MACRO_CONFIG_STR
	...
	if(!m_pStorage->RenameFile(aConfigFileTmp, CONFIG_FILE, IStorageEngine::TYPE_SAVE))
	{
