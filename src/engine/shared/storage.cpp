...
#define CLIENT_EXEC "MRPG"
#define SERVER_EXEC "MRPG-Server"
...
class CStorage : public IStorageEngine
{
public:
	...
	static IStorageEngine *Create(int StorageType, int NumArgs, const char **ppArguments)
	{
...
void IStorageEngine::StripPathAndExtension(const char *pFilename, char *pBuffer, int BufferSize)
{
...
IStorageEngine *CreateStorage(int StorageType, int NumArgs, const char **ppArguments)
{
	return CStorage::Create(StorageType, NumArgs, ppArguments);
}

IStorageEngine *CreateLocalStorage()
{
	CStorage *pStorage = new CStorage();
	if(pStorage)
	{
...
IStorageEngine *CreateTempStorage(const char *pDirectory)
{
	CStorage *pStorage = new CStorage();
	if(!pStorage)
	{
